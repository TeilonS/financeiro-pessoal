...
/**
 * Parser para extrato CSV da Caixa Econômica Federal.
 *
 * Formatos suportados:
 *
 * Formato A — coluna única Valor (positivo=crédito, negativo=débito):
 *   Data;Histórico;Descrição;Valor;Saldo
 *   01/01/2026;Crédito em C/C;PIX RECEBIDO;100,00;1000,00
 *   02/01/2026;Débito;Compra;-50,00;950,00
 *
 * Formato B — colunas separadas Crédito / Débito:
 *   Data;Histórico;Descrição;Crédito;Débito;Saldo
 *   01/01/2026;Crédito em C/C;PIX RECEBIDO;100,00;;1000,00
 *   02/01/2026;Débito;Compra;;50,00;950,00
 *
...
public class CsvCaixaParser {
        ...
        for (String linha : linhas) {
            ...
            if (!cabecalhoEncontrado) {
                ...
                for (String col : cols) {
                    ...
                    if (c.contains("valor") || c.contains("credito") || c.contains("debito")
                            || c.contains("hist") || c.contains("descri")) temValor = true;
                }
                ...
                for (int i = 0; i < cols.length; i++) {
                    ...
                    if (c.startsWith("credito") || c.startsWith("crédito")) colCredito = i;
                    if (c.startsWith("debito") || c.startsWith("débito")) colDebito = i;
                }

                boolean formatoB = colCredito >= 0 && colDebito >= 0;
                if (colData >= 0 && (colValor >= 0 || formatoB)) cabecalhoEncontrado = true;
                continue;
            }
    ...
    private char detectarSeparador(String[] linhas) {
        for (String linha : linhas) {
            ...
            if (l.contains("data") && (l.contains("valor") || l.contains("hist") || l.contains("credito"))) {
    ...
    private BigDecimal parsearValorBR(String s) {
        s = s.replaceAll("[\\s\"R$]", "").replace("R$", "").replace("+", "");
