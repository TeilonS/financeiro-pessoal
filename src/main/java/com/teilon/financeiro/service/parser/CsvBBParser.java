...
/**
 * Parser para extrato CSV do Banco do Brasil.
 *
 * Formatos suportados:
 *
 * Formato A — colunas Crédito / Débito separadas:
 *   Data;Histórico;Docto;Crédito(R$);Débito(R$);Saldo(R$)
 *   01/01/2026;CRED PRIV TRF;123456;100,00;;1000,00
 *   02/01/2026;DÉBITO PIX;654321;;50,00;950,00
 *
 * Formato B — coluna única Valor (positivo=crédito, negativo=débito):
 *   Data;Histórico;Documento;Valor;Saldo
 *   01/01/2026;CRED PRIV TRF;123456;100,00;1000,00
 *   02/01/2026;DÉBITO PIX;654321;-50,00;950,00
 *
 * O arquivo costuma ter linhas de cabeçalho/metadados antes da tabela.
 * Encoding: ISO-8859-1 ou UTF-8.
 */
@Component
public class CsvBBParser {
        ...
        for (String linha : linhas) {
            ...
            if (!cabecalhoEncontrado) {
                ...
                for (int i = 0; i < cols.length; i++) {
                    ...
                    if (c.contains("historico") || c.contains("hist")) colDescricao = i;
                    if (c.equals("valor")) colValor = i;
                    if (c.startsWith("credito")) colCredito = i;
                    if (c.startsWith("debito")) colDebito = i;
                }

                boolean formatoA = colCredito >= 0 && colDebito >= 0;
                if (colData >= 0 && (colValor >= 0 || formatoA)) cabecalhoEncontrado = true;
                continue;
            }
