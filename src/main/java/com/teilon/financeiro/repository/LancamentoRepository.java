...
public interface LancamentoRepository extends JpaRepository<Lancamento, Long> {
    ...
    @Query("SELECT l FROM Lancamento l JOIN FETCH l.categoria WHERE l.usuario = :usuario AND l.tipo = :tipo")
    List<Lancamento> findAllByUsuarioAndTipo(@Param("usuario") Usuario usuario, @Param("tipo") TipoTransacao tipo);

    @Query("SELECT l FROM Lancamento l JOIN FETCH l.categoria WHERE l.id = :id AND l.usuario = :usuario")
